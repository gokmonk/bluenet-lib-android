...
//				delayedDisconnect(null);
	 ...
	 */
	public void togglePower(final IBooleanCallback callback) {
		readPwm(new IIntegerCallback() {
			@Override
			public void onSuccess(int result) {
				if (result > 0) {
					powerOff(new IStatusCallback() {
						@Override
						public void onSuccess() {
	 ...
	 * @param callback callback which will be informed about success or failure
	 */
	public void togglePower(final String address, final IBooleanCallback callback) {
		getHandler().post(new Runnable() {
			@Override
			public void run() {
